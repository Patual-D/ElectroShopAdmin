...
// Se hace un JOIN con la tabla 'servicio' para contar las compras
$from = "FROM tipo_servicio TS LEFT JOIN servicio S ON TS.id_tipo_servicio = S.id_tipo_servicio";
...
if ($busqueda !== "") {
    // La búsqueda funciona por nombre del servicio o descripción
    $where = "WHERE 
        TS.nombre_servicio LIKE ? OR
        TS.descripcion_servicio LIKE ?";
...
$sqlTotal = "SELECT COUNT(*) as total FROM tipo_servicio " . ($where ? "WHERE nombre_servicio LIKE ? OR descripcion_servicio LIKE ?" : "");
if ($where) {
    ...
    $total_result = $conn->query("SELECT COUNT(*) as total FROM tipo_servicio");
}
...
$sql = "SELECT TS.*, COUNT(S.id_servicio) as total_compras 
        $from 
        $where 
        GROUP BY TS.id_tipo_servicio
        ORDER BY TS.id_tipo_servicio LIMIT ?, ?";
...
<head>
  <meta charset="utf-8">
  <title>Admin Panel - Tipos de Servicio</title>
...
<body>
  <div class="navbar navbar-dark bg-dark flex-column p-0">
    ...
    <div class="container-fluid w-100 d-flex flex-wrap pb-2" style="gap: 5px;">
        ...
        <a href="tipo_servicio.php" class="btn btn-primary btn-sm flex-grow-1">Tipos Servicios</a>
  ...
  <main class="container py-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
      <h2 class="h4 m-0">Tipos de Servicio</h2>
      ...
      <tbody>
        <?php if ($result && $result->num_rows > 0): ?>
          <?php while ($row = $result->fetch_assoc()) { ?>
            <tr>
              <td><?php echo htmlspecialchars($row['id_tipo_servicio']); ?></td>
              <td><?php echo htmlspecialchars($row['nombre_servicio']); ?></td>
              <td><?php echo htmlspecialchars($row['descripcion_servicio']); ?></td>
              <td>$<?php echo htmlspecialchars(number_format($row['costo'], 2)); ?></td>
              <td><?php echo htmlspecialchars($row['total_compras']); ?></td>
              <td>
                <a href="update/tipo_servicio.php?id=<?php echo urlencode($row['id_tipo_servicio']); ?>&pagina=<?php echo $pagina; ?>" class="btn btn-warning btn-sm">Editar</a>
              </td>
            </tr>
          <?php } ?>
        <?php else: ?>
          <tr>
            <td colspan="6" class="text-center text-muted">No se encontraron tipos de servicio.</td>
          </tr>
        <?php endif; ?>
      </tbody>
    </table>
    ...
    <div class="d-flex justify-content-between align-items-center">
      ...
      <a href="create/tipo_servicio.php" class="btn btn-primary">Registrar Tipo de Servicio</a>
    </div>
