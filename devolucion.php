...
// Se unen 4 tablas para obtener toda la información necesaria
$fromJoin = "
    FROM devolucion D
...
if ($busqueda !== "") {
    // La búsqueda funciona por nombre de producto, nombre de cliente, ID de compra o ID de devolución
    $where = "WHERE 
    ...
    $like = "%{$busqueda}%";
    $params = [$like, $like, $busqueda, $busqueda];
    $types = "ssii";
}
...
if ($where) {
    ...
    $total_result = $conn->query("SELECT COUNT(*) as total FROM devolucion");
}
...
$sql = "$select $fromJoin $where ORDER BY D.id_devolucion DESC LIMIT ?, ?";
...
<head>
  <meta charset="utf-8">
  <title>Admin Panel - Devoluciones</title>
...
<body>
  <div class="navbar navbar-dark bg-dark flex-column p-0">
    ...
    <div class="container-fluid w-100 d-flex flex-wrap pb-2" style="gap: 5px;">
        ...
        <a href="devolucion.php" class="btn btn-primary btn-sm flex-grow-1">Devoluciones</a>
  ...
  <main class="container py-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
      <h2 class="h4 m-0">Gestión de Devoluciones</h2>
      <form class="d-flex" method="get" action="">
        <input type="hidden" name="pagina" value="1">
        <input class="form-control me-2" type="search" name="q" placeholder="Buscar por producto, cliente, ID..." value="<?php echo htmlspecialchars($busqueda); ?>">
    ...
    <table class="table table-striped align-middle">
      ...
      <tbody>
        <?php if ($result && $result->num_rows > 0): ?>
          <?php while ($row = $result->fetch_assoc()) { ?>
            <tr>
              ...
              <td>
                <span class="badge <?php echo $row['con_garantia'] ? 'bg-success' : 'bg-secondary'; ?>">
                  <?php echo $row['con_garantia'] ? 'Sí' : 'No'; ?>
                </span>
              </td>
            </tr>
          <?php } ?>
        <?php else: ?>
          <tr>
            <td colspan="7" class="text-center text-muted">No se encontraron devoluciones.</td>
          </tr>
        <?php endif; ?>
      </tbody>
    </table>
