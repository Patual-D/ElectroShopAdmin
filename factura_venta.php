...
// Se une 'factura' con 'cliente'
$fromJoin = "FROM factura F LEFT JOIN cliente CL ON F.id_cliente = CL.id_cliente";

// Construcción del WHERE dinámico, con la condición FIJA de que el tipo sea 'venta'
$baseWhere = "WHERE F.tipo_factura = 'venta'";
$where = $baseWhere;
$params = [];
$types = "";

if ($busqueda !== "") {
    // La búsqueda funciona por nombre de cliente, método de pago o ID de la factura
    $where .= " AND ( 
...
if ($busqueda !== "") {
    ...
    $total_result = $conn->query("SELECT COUNT(*) as total FROM factura WHERE tipo_factura = 'venta'");
}
...
<head>
  <meta charset="utf-8">
  <title>Admin Panel - Facturas de Venta</title>
...
<body>
  ...
  <div class="navbar navbar-dark bg-dark flex-column p-0">
    ...
    <div class="container-fluid w-100 d-flex flex-wrap pb-2" style="gap: 5px;">
        ...
        <a href="factura_venta.php" class="btn btn-primary btn-sm flex-grow-1">Facturas Ventas</a>
  ...
  <main class="container py-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
      <h2 class="h4 m-0">Facturas de Venta</h2>
        ...
        <?php if ($result && $result->num_rows > 0): ?>
          <?php while ($row = $result->fetch_assoc()) { ?>
            <tr>
              ...
              <td>
                <a href="detail/factura.php?id=<?php echo urlencode($row['id_factura']); ?>" class="btn btn-info btn-sm">Ver Detalles</a>
              </td>
            </tr>
          <?php } ?>
        <?php else: ?>
          <tr>
            <td colspan="6" class="text-center text-muted">No se encontraron facturas de venta.</td>
          </tr>
        <?php endif; ?>
      </tbody>
    </table>
